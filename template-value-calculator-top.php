  ...
  <script>
    $(document).ready(function() {
      $('[data-onchange').on('change', function() {
        ...
        const noOfEmployees = parseInt($('#number-of-employees').val());
        const staffAvgHourlyWage = parseInt($('#staff-avg-hourly-wage').val());
        const numberOfAnnualContractRenewals = parseInt($('#number-of-annual-contract-renewals').val());
          
        // get step 2 inputs
        const contractCreationTime = parseInt($('#contract-creation-time').val());
        const bookBindingSealingTime = parseInt($('#book-binding-sealing-time').val());
        const shippingTime = parseInt($('#shipping-time').val());
          
        // get step 3 inputs
        const dataEntry = parseInt($('#data-entry').val());
        const contractFiling = parseInt($('#contract-filing').val());
